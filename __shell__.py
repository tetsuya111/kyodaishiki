	...
	def execQuery(self,query,output):
		...
		if query.command in Command.QUIT:
			...
			if args["file"]:
				...
				return self.beginf(fname)
		else:
			#output.write("Quit is \"q\" or \"quit\".\n")
			return None
	def __begin__(self,input_,output,precmd=None,postcmd=None):
#this loop while input_ reach EOF or execQuery raise SystemExit.
#this is iterator.yield res as execQuery
#res -1 if return as exception.
		while True:
			try:
				if precmd:
					precmd()
	...
	def beginf(self,fname):
		if not os.path.exists(fname):
			return 0
		with open(fname,"r") as f:
			return self.__begin__(f,self.stdout)
	def shell(self):
...
class DBShell(BaseShell):
	...
	def execQuery(self,query,output):
		if query.command in Command.SEARCH:
			...
			return self.execQuery(Query(("search","-t",*query.data)),output)
...
and it has alias.txt.this is execed before enter shell,as alias <line>...
	...
	def __init__(self,dname=None,prompt=BaseShell.PROMPT):
		...
		if self.dname:
			init_alias=os.path.join(self.dname,self.INIT_ALIAS)
			_util.touch(init_alias)
			self.execAliasf(init_alias,self.null)
	def __begin__(self,input_,output):
		while True:
			try:
				output.flush()
				if not input_.readable():
...
#output is str.
		...
		output=None
		mode=""
		for pipe in (self.OUTPUT_PIPE,self.OUTPUT_ADD_PIPE):
			if pipe in query :
				index_of_pipe=list(query).index(pipe)
				output=query[index_of_pipe+1]
				query=query[:index_of_pipe]
	...
	def execAliasf(self,fname,output):
		if not os.path.exists(fname):
			return
		with open(fname,"r") as f:
			for line in f:
